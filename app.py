...
def build_keywords(query: str) -> List[str]:
    ...
    for p in parts:
        if " " in p:
            ...
            variants.add(p.replace(" ", ""))
    return [x.lower() for x in variants if x]

def match_entry(entry_text: str, keywords: List[str]) -> bool:
    if not keywords:
        return True
    hay = entry_text.lower()
...
async def fetch_text(session: aiohttp.ClientSession, url: str, timeout: int = 20) -> str:
    async with session.get(url, timeout=timeout, headers={"User-Agent": "physics-topic-aggregator/1.0"}) as resp:
...
async def fetch_and_filter_feed(session: aiohttp.ClientSession, url: str, keywords: List[str]) -> List[Dict[str, Any]]:
    try:
        text = await fetch_text(session, url)
    except Exception:
        return []
...
async def aggregate(topic: str) -> List[Dict[str, Any]]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        feed_tasks = [
            fetch_and_filter_feed(session, url, keywords) for url in cfg["normal_feeds"]
        ]
...
# FastAPI app
# -----------------------
app = FastAPI(title="Physics Topic RSS Aggregator", version="1.0")
